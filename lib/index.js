...
function getProjectRootFromTargetFile(targetFile) {
  ...
  if (
    parts[parts.length - 1] === 'Gopkg.lock' ||
    parts[parts.length - 1] === 'vendor.conf') {
...
function recursivelyBuildPkgTree(
  ...
  if (isRoot || isProjSubpkg) {
    pkg.version = '';
  } else if (!getVersion(pkg.name, lockedVersions)) {
    pkg.version = '';
    // TODO: warn or set to "?" ?
  } else {
    pkg.version = getVersion(pkg.name, lockedVersions);
  }
...
function parseConfig(root, targetFile) {
  ...
  switch (pkgManager) {
    ...
    case 'vndr': {
      config = parseVndr(root, targetFile);
      break;
    }
    default: {
      throw new Error('Unsupported file: ' + targetFile);
...
function parseVndr(root, targetFile) {
  var config = {
    ignoredPkgs: [],
    lockedVersions: {},
  };
  ...
  try {
    var vndrTxt = fs.readFileSync(path.join(root, targetFile), 'utf8');

    for (var line of vndrTxt.split(/\r?\n/)) {
      // strip comments
      ...
      var [name, version] = stripped.split(/\s/);
      config.lockedVersions[name] = {
        name,
        version: semver.valid(version) ? version : '#' + version,
      };
    }
...
function getVersion(name, lockedVersions) {
  if (lockedVersions[name]) {
    return lockedVersions[name].version;
  }
  ...
  return getVersion(parts.slice(0, parts.length - 1).join('/'), lockedVersions);
}
